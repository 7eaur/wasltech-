...
function updateTime() {
    ...
    if (timeDisplay) {
        const now = new Date();
        timeDisplay.textContent = now.toLocaleTimeString('ar-DZ', {
            hour: '2-digit',
            minute: '2-digit',
            hour12: false
        });
...
contactForm.addEventListener('submit', (e) => {
    ...
    btn.innerHTML = '<i class="fas fa-circle-notch animate-spin"></i> جارٍ الإرسال...';
    btn.disabled = true;

    setTimeout(() => {
        showToast('تم إرسال رسالتك بنجاح! سأتواصل معك قريباً.');
        e.target.reset();
